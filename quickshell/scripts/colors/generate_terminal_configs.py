...
def generate_btop_config(colors, output_path):
    ...
    # primary = colors.get("primary_fixed_dim") or primary
    # secondary = colors.get("secondary_fixed_dim") or secondary
    # tertiary = colors.get("tertiary_fixed_dim") or tertiary
    error = colors.get("error")
        # -----------------------------
    # Safe helper to avoid writing "None" in theme
    def safe_color(val, default="#FFFFFF"):
...
"""
    ...
    if not btop_path.exists():
        btop_path.parent.mkdir(parents=True, exist_ok=True)
        btop_path.write_text(
            'color_theme = "ii-auto"\n'
            'theme_background = true\n'
            'truecolor = true\n'
        )
        print(f"\u2713 Created btop.conf with ii-auto theme")
    else:
        print(f"\u2713 btop.conf already exists — leaving it unchanged")
    # if btop_path.exists():
