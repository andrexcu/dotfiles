...
Item {
    ...
    property string pendingArtUrl: ""
        ...
        Timer {
            ...
            running: true
            onTriggered: {
                let current = Players.active

                // 1️⃣ Only use current if it has media
                if (!current || !Players.hasMedia(current)) {
                    // fallback: find first player with media
                    const candidate = Players.list.find(p => Players.hasMedia(p))
                    if (!candidate) return
                    current = candidate
                }

                // 2️⃣ Always keep a safePlayer reference
                if (root.safePlayer !== current) {
                    root.safePlayer = current
                }

                // 3️⃣ Update metadata snapshots
                ...
                else root.currentVisibleTitle = Translation.tr("No media playing")
                
                if (newArtist) root.currentVisibleArtist = newArtist
                else root.currentVisibleArtist = ""

                // 4️⃣ Update artwork if metadata is ready
                if (newTitle) {
                    updateArtAndInfo()
                }
            }
    ...
    if (!safePlayer) return ""
    return isYtMusicPlayer ? YtMusic.currentThumbnail
                            : (safePlayer.trackArtUrl || "")
    }   
        ...
        onExited: (exitCode) => {
            if (exitCode === 0) {
                if (pendingArtUrl !== targetFile) {
                    // A new track came in while we were downloading, start next download
