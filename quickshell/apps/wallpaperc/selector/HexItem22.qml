...
// }
    ...
    // property real targetX:
    // controller.currentIndex.baseX(itemIndex)
    // + (entering ? 0 : (hoverRippleOff ? 0 : hoverRipple.x))

    // property real targetY:
    // controller.currentIndex.baseY(itemIndex)
    ...
    // }
    // y: controller.currentIndex.baseY(itemIndex) + ripple.y + originFixY * (1 - scale)
    // property real layoutX: 0
    // property real layoutY: 0
    // targetX: !rippleOff ?
    // controller.currentIndex.baseX(itemIndex) + layoutX + ripple.x : controller.currentIndex.baseX(itemIndex)

    // targetY: !rippleOff ?
    // controller.currentIndex.baseY(itemIndex) + layoutY + ripple.y : controller.currentIndex.baseY(itemIndex)

   
    // Behavior on scale {
    //     // enabled: controller.currentIndex.firstUpdateDone
    //     NumberAnimation {
    //         duration: 400
    //         easing.type: Easing.BezierSpline
    //         easing.bezierCurve: [0.25, 0.1, 0.25, 1.0]
...
// }
    ...
    onIsSelectedChanged: {
        ...
        //  console.log(controller.currentIndex.currentItem)
    ...
    MouseArea {
        id: mouseArea
        anchors.fill: parent
        hoverEnabled: controller.currentIndex.listViewShown ? true : false
        ...
        onClicked: {
            ...
            flickRef.forceActiveFocus() 
            // Qt.callLater(() => controller.currentIndex.forceActiveFocus())
        }
