...
// 	spacing: 16
 ...
 Item {
	...
	// width: flick.contentItem.width
    // height: flick.contentItem.height
	...
	// }
			...
			// visible: wallpaperController.cardVisible
			// anchors.fill: parent   // OR ListView default full width
			...
			NumberAnimation {
				...
				onStarted: {	
					...
					flick.currentIndex = 0
					// flick._selectedCol = 10
					// flick._selectedRow = 10
					// flick.globalShiftX = 0
					// flick.globalShiftY = 0
				}
			...
			property real _hexW: cellWidth
			property real _hexH: cellHeight
			...
			property real _gridContentH: (_rows - 1) * _stepY + _hexH + _hexH / 2
			property real _yOffset: Math.max(0, (height - _gridContentH))
			property real _visibleBandX: (wallpaperController.hexCols - 1) * _stepX + _hexW
			property real _fadeZoneX: (width - _visibleBandX) / 2
			
			// property real _visibleBand: (_cols - 1) * _stepX + _hexW
			// property real _fadeZone: (width - _visibleBand) / 2

			// property real _visibleBandX: (_cols - 1) * _colStep + hCellWidth
			// property real _fadeZoneX: (width - _visibleBandX) / 2
			
			property real _visibleBandY:
				(_rows - 1) * _rowStep + vCellWidth

			property real _fadeZoneY:
				(height - _visibleBandY) / 2
			...
			// property real _contentWidth: Math.ceil(filteredModel / _cols) * _stepX
		    // property real _contentHeight: Math.ceil(filteredModel / _rows) * _stepY
			
			// contentWidth:  _contentWidth
			// contentHeight: _contentHeight
			// property  real _contentWidth: _cols * _colStep
			// property real _contentHeight: Math.ceil(filteredModel / _cols) * _rowStep
			...
			// Math.ceil(filteredModel / _cols) * _rowStep
			...
			// property real _colStep: 1.5 * _r + _gridSpacing
			// property real _rowStep: hCellWidth + _gridSpacing
	...
	// 		onMovementEnded: {
	// 			  var col = Math.round(contentX / _stepX)
...
// }
			onMovementEnded: {
				...
				// flick.currentIndex = Math.round(flick.currentIndex)
			}
				// if (isHorizontal) {
				// 	contentX = Math.round(contentX / _colStep) * _colStep
				// } else {
			...
			// 							+ (visibleRows - 1) * rowStep
					...
					// width: isHorizontal 
					// ? flick._hexW + (_cols - 1) * _colStep 
					// : cardContainer.width
					// height: isHorizontal
					// ? cardContainer.height 
					// : flick._hexH + (_rows - 1) * _rowStep
					// width: cardContainer.width * 0.5
					...
					property real _gridInset: 100
// 					property real clusterWidth:
//     flick._hexW + (_cols - 1) * _colStep

// property real actualWidth:
//     width

// property real hOffset:
//     (actualWidth - clusterWidth) / 2

					property real hOffset: Math.max((
						((WallpaperService.columns - 1) * _colStep + flick.hCellWidth) - hGridWidth) / 2, 0)
					
					property real vOffset:
					Math.max((
						((WallpaperService.rows - 1) * _rowStep + flick.vCellHeight) - vGridHeight) / 2, 0)
        ...
        onWheel: function(wheel) {
          var step = 1
		// step = Math.max(1, Math.floor(flick.length * 0.05))
          if (wheel.angleDelta.y > 0 || wheel.angleDelta.x > 0) {
            flick.currentIndex = Math.max(0, flick.currentIndex - step)
            flick._selectedCol = flick.currentIndex
          } else if (wheel.angleDelta.y < 0 || wheel.angleDelta.x < 0) {
            flick.currentIndex = Math.min(flick.count - 1, flick.currentIndex + step)
            flick._selectedCol = flick.currentIndex
          }
        }
...
// header: Item { width: (flick.width - flick._stepX) / 2  }
// footer: Item { width: (flick.width - flick._stepX) / 2  }
		...
		Keys.onPressed: function(event) {
			...
			var maxRow = Math.min(_rows, filteredWallpapers.length - _selectedCol * _rows) - 1
		...
		// 	}
		// 	if (event.key === Qt.Key_Down && !(event.modifiers & Qt.ShiftModifier)) {
		// 	var maxRow = Math.min(_rows, filteredModel - _selectedCol * _rows) - 1
		// 	if (_selectedRow < maxRow) _selectedRow++
		// 	event.accepted = true
		// 	return
		// 	}
		...
		function smartScroll(i, oldIndex) {
			...
			if (isHorizontal) {
				...
				if (rowBottom > viewBottom) {
					...
					// property real centerOffsetX:
					// 	Math.max((width - _contentWidth) / 2, 0)

					// property real centerOffsetY:
					// 	Math.max((height - _contentHeight) / 2, 0)
					...
					delegate: Item {
						...
						// readonly property bool _insideViewX: _hexCenterX > -flick.effectiveCellStepX && _hexCenterX < flick.width + flick.effectiveCellStepX
						...
						readonly property bool _visible:
						isHorizontal
							? _insideViewX && !_nearEdgeX
						...
						readonly property bool _insideViewY:
						y + height >= flick.contentY - flick._rowStep &&
						y <= flick.contentY + flick.height + flick._rowStep
						readonly property bool _nearEdgeY:
						_hexCenterY < flick._fadeZoneY ||
						_hexCenterY > (flick.height - flick._fadeZoneY)

					
						// readonly property bool _insideViewY:
						// _hexCenterY > -flick.effectiveCellStepY &&
						// _hexCenterY < flick.height + flick.effectiveCellStepY
						// readonly property bool _visible: isHorizontal 
						// ? _insideViewX && !_nearEdgeX
						// : _insideViewY && !_nearEdgeY
					   ...
					   readonly property bool _nearLeft: _hexCenterX < flick.width / 2
						Repeater {
							...
							delegate: HexItem {
								...
								// property bool _inView: hexDelegate._visible
								...
								property bool _isSelected: hexDelegate.hColIndex === flick._selectedCol && hRowIndex === flick._selectedRow
								...
								scale: hexDelegate._hexScale
								opacity: hexDelegate._hexScale < 0.01 ? 0 : 1
								visible: hexDelegate._hexScale > 0 ? true : false
								...
								// 			: 0) : 0
								viewY: flick._yOffset + hRowIndex * flick._stepY + (hexDelegate.hColIndex % 2 !== 0 ? flick._hexH / 2 : 0)
								// y: flick._yOffset + hRowIndex * flick._stepY + (hexDelegate.hColIndex % 2 !== 0 ? flick._hexH / 2 : 0)
...
//         : 0)
								...
								innerParallaxX: {
								var viewCenterX = flick.width / 2
								var normalized = (hexDelegate._hexCenterX - viewCenterX) / Math.max(1, viewCenterX)
								return -normalized * flick._r * 1.15
								}
