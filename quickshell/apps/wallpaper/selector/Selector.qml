...
function viewLeftCol() {
    return Math.floor(contentX / _stepX)
}

function viewRightCol() {
    return viewLeftCol() + hexCols - 1
}
...
property real columns:
    Math.ceil(filteredWallpapers.count / _rows)
...
Component.onCompleted: Qt.callLater(applyCenter)

onWidthChanged: applyCenter()
onCountChanged: applyCenter()

function applyCenter() {
    if (_rows <= 0 || filteredWallpapers.count <= 0)
        return

    var columns = Math.ceil(filteredWallpapers.count / _rows)
    var gridWidth = columns * _stepX
    var centerOffset = Math.max((width - gridWidth) / 2, 0)

    contentX = -centerOffset
}
      ...
      delegate: Item {
        ...
        Repeater {
          ...
          delegate: HexItem {
            ...
            // x: 0
			// x: hexCol.colIdx * hexListView._stepX + hexListView.centerOffset
