 ...
 Item {
	...
	// 	clip: false
		...
		Flickable {
			...
			// property real _contentWidth: Math.ceil(filteredWallpapers.length / columns) * _colStep
			...
			Component.onCompleted: {
				...
				// 	id: flick
					...
					property int poolSize: 40

					property int startIndex: flick.hStartIndex
					property int endIndex: startIndex + poolSize
				...
				function updatePool() {
					console.log("updatePool start", filteredWallpapers.length, poolSize)
					const start = flick.hStartIndex

					for (let i = 0; i < poolSize; i++) {
						const dataIndex = start + i
						const item = wallpaperRepeater.itemAt(i)

						if (!item) continue

						if (dataIndex < filteredWallpapers.length) {
							const col = Math.floor(dataIndex / flick._rows)
							const row = dataIndex % flick._rows

							const itemY =
								row * flick.effectiveCellStepY +
								(col % 2 ? flick.effectiveCellStepY * 0.5 : 0)

							item.realIndex = dataIndex
							item.itemData = filteredWallpapers[dataIndex]

							item.x = flick.hOffset + col * flick._colStep
							item.y = flick.gridOffsetY + itemY
					...
					property var poolItems: []
					Repeater {
						id: wallpaperRepeater
						model: flick.poolSize
  ...
  onItemAdded: (item, index) => {
        flick.poolItems[index] = item
    }

    onItemRemoved: (item, index) => {
        flick.poolItems[index] = null
    }
						Item {
							id: hexItem
							z: isSelected ? 999 : 1
							property bool isSelected: wallpaperController.currentIndex === realIndex
							...
							property bool _inView: 
								((realIndex >= flick.hStartIndex) &&
								(realIndex < flick.hEndIndex))
							...
							property real col: Math.floor(realIndex / flick.visibleRows)
							property real row: realIndex % flick.visibleRows
							...
							function computeShiftX() {
								var selIndex = wallpaperController.currentIndex
								if (realIndex === selIndex) return 0
								...
								var row = Math.floor(realIndex / cols)
								var col = realIndex % cols
						...
						property real shiftY: 0
							...
							*/
							property bool isNeighbor: {
								...
								var row = Math.floor(realIndex / totalColumns)
								var col = realIndex % totalColumns
								var selectedRow = Math.floor(selected / totalColumns)
								var selectedCol = selected % totalColumns

								if (realIndex === selected) return false  // selected itself is not a neighbor
							...
							property int gen: {
								...
								if (realIndex === sel) return 0

								var a = getHexPos(realIndex)
								var b = getHexPos(sel)
...
// )
							 ...
							 Item {
								...
								Image {
									...
									anchors.centerIn: parent
									asynchronous: true
									property string thumbName: thumbnailPaths[itemData] || ""
								    ...
								    layer.effect: MultiEffect {
										blurEnabled: true
										blur: wallpaperController.currentIndex === realIndex && 
...
// }
								...
								Rectangle {
									...
									opacity: wallpaperController.currentIndex === realIndex
									? 0.6: 0
							...
							MouseArea {
								...
								onClicked: {
									wallpaperController.currentIndex = realIndex
