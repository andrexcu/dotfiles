...
Scope {
	...
	Timer {
		...
		onTriggered: {
			if (wallpaperController.currentSelected) {
				wallpaperController.currentSelected.visualScale = 1.15
	...
	function flipHex() {
		...
		if (wPrevious && wPrevious !== wSelected) {
			...
			wPrevious.visualScale = 1
			wPrevious.fadeOpacity = 1
		...
		vw.flipAngle = 0
		selectedItem.visualScale = 0.25
		selectedItem.fadeOpacity = 0
 ...
 Item {
    ...
    //   anchors.bottom: cardContainer.bottom
      ...
      anchors.right: cardContainer.right
			...
			height: flick.cellHeight + (visibleRows - 1) * _rowStep

			// height: flick.cellHeight + (3 - 1) * _rowStep
			...
			// contentHeight: flick.childrenRect.height
			...
			function updateScales() {
				...
				for (var i = 0; i < wallpaperRepeater.count; i++) {
					...
					if (!item) continue
					var itemTop = item.y
					var itemBottom = item.y + flick.cellHeight * 0.6
				...
				MouseArea {
					...
					Repeater {
						...
						Item {
							...
							property real baseX: flick.itemX(index)
							property real baseY: flick.itemY(index)
