...
//     (columns - 1) * _colStep + cellWidth
			property real _contentWidth: Math.ceil(filteredWallpapers.length / flick.columns) * _colStep
		    // property real _contentHeight: Math.ceil(filteredWallpapers.length / flick.visibleRows) * _rowStep
			...
			Connections {
				...
				// 	id: flick
					...
					delegate: Item {
							...
							property var itemData: filteredWallpapers[index]
							...
							property real baseY: (((flick.height ? flick.height : 0) - flick.hGridHeight) / 2)
										+ index * flick.effectiveCellStepY
										+ (index % 2
