...
// 	spacing: 16
 ...
 Item {
	...
	ListView {
      ...
      visible: wallpaperController.cardVisible
		Component.onCompleted: {
			console.log("r", hexListView._r)
			console.log("stepY", hexListView._stepY)
			console.log("rows", hexListView._rows)
		}
	...
	//   highlightRangeMode: ListView.ApplyRange
      highlightRangeMode: ListView.StrictlyEnforceRange
		header: Item {
			width: Math.max(0,
				(hexListView.width - hexListView._visibleBand) / 2
				+ hexListView._stepX
			)
		}
    ...
    //   footer: Item { width: (hexListView.width - hexListView._hexW) / 2 }
      ...
      MouseArea {
        ...
        onWheel: function(wheel) {
          var step = 1
		// step = Math.max(1, Math.floor(hexListView.count * 0.05))
      ...
      Keys.onPressed: function(event) {
        ...
        if (event.key === Qt.Key_Down && !(event.modifiers & Qt.ShiftModifier)) {
          var maxRow = Math.min(_rows, filteredWallpapers.count - _selectedCol * _rows) - 1
      ...
      delegate: Item {
        ...
        Repeater {
          ...
          delegate: HexItem {
            ...
            // colors: wallpaperController.colors
            // service: wallpaperController.selectorService
			...
			parallaxX: {
              ...
              return -normalized * hexListView._r * 1.15
            }
            // parallaxY: {
