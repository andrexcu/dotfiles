...
Item {
    ...
    function startDrag(index, mouseY) {
        ...
        const item = repeater.itemAt(dragIndex)
        if (item && item.contentContainer) {
            draggedItemRef = item  // keep reference

            draggedWidgetContainer.width = item.width
            draggedWidgetContainer.height = item.height
            draggedWidgetContainer.visible = true

            // Reparent contentContainer to overlay
            draggedWidgetContainer.content = item.contentContainer
            item.visible = false
        }
    }
   ...
   function updateDrag(mouseY) {
        ...
        // Move overlay
        draggedWidgetContainer.y = dragCurrentY - dragStartY

        // Update hoverIndex for other items in layout
        let accY = 0
        for (let i = 0; i < repeater.count; i++) {
            const item = repeater.itemAt(i)
    ...
    function endDrag(): void {
        ...
        // Grab the item BEFORE moving anything
        const draggedItem = repeater.itemAt(dragIndex)
        if (draggedItem) {
            // put the dragged widget back in ColumnLayout
            draggedItem.contentContainer.parent = draggedItem
            draggedItem.visible = true
        }

        // Only now move the model if necessary
        if (hoverIndex >= 0 && dragIndex !== hoverIndex) {
            moveWidget(dragIndex, hoverIndex)
        }

        draggedWidgetContainer.visible = false
        _resetDrag()
    }
    ...
    ColumnLayout {
        ...
        Repeater {
            ...
            delegate: Item {
               ...
               transform: Translate {
                    y: widgetWrapper.isBeingDragged
                        ? root.dragCurrentY - root.dragStartY  // follow mouse relative to drag start
                        : widgetWrapper.displacementY
                ...
                Item {
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            ...
                            // }
                    onPressed: (mouse) => {
                            // Record where the mouse started relative to the ColumnLayout
                            const mouseY = mapToItem(column, mouse.x, mouse.y).y
                            root.dragStartY = mouseY
                            root.dragCurrentY = mouseY
                            root.startDrag(widgetWrapper.index, mouseY)
                        }

                            onPositionChanged: (mouse) => {
                                const mouseY = mapToItem(root, mouse.x, mouse.y).y
                                root.dragCurrentY = mouseY

                                if (root.dragIndex === widgetWrapper.index) {
                                    // move the dragged widget visually
                                    root.updateDrag(mouseY)
                                }
                            }
                            // onPositionChanged: (mouse) => {
    ...
    Item {
        id: dragOverlay
        anchors.fill: parent
        z: 100   // make sure it’s above everything else

        Item {
            id: draggedWidgetContainer
            visible: false   // only visible while dragging
            width: 0
            height: 0
        }
    }
