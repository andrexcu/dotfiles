...
Singleton {
    ...
    // Process {
    //     id: fetcher
    //     command: ["/usr/bin/echo", "{\"current_condition\":[{\"temp_C\":25,\"weatherCode\":113}]}"]
    //     stdout: StdioCollector {
    //         onStreamFinished: {
    //             console.log("[DEBUG] Fetcher stdout:", text)
    //             const parsed = JSON.parse(text)
    //             root.refineData({ current: parsed.current_condition[0] })
    //         }
    //     }
    // }
    // Weather fetcher
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                try {
                    ...
                    const normalized = {
                        current: rootData?.current_condition?.[0] ?? rootData?.current,
                        astronomy: rootData?.weather?.[0]?.astronomy?.[0] ?? rootData?.astronomy?.[0]
                    }
                    // const normalized = {
