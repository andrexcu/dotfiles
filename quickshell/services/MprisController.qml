...
Singleton {
	...
	property MprisPlayer selectedPlayer: trackedPlayer // default
    ...
    // // Track existing player references
    // const existing = new Set(players)

    // // Only add new players
    // for (const p of Mpris.players.values) {
    //     if ((isRealPlayer(p) || _isInGracePeriod(p)) && !existing.has(p)) {
...
//     for (const p of Mpris.players.values) {
//         // Only include real players or ones in grace period
//         if (isRealPlayer(p) || _isInGracePeriod(p)) {
//             // Skip players that have no metadata or track info
//             const hasTrack = p.metadata?.title || p.metadata?.artUrl || p.trackTitle;
...
//     // Stable sort by firstSeen
//     newList.sort((a, b) => a.firstSeen - b.firstSeen);

//     // Mutate the array in place
//     players.length = 0;
//     for (const p of newList) players.push(p);
// }
	...
	function isRealPlayer(player) {
		...
		if (
			(name.includes("chromium") || name.includes("chrome") || name.includes("firefox") || name.includes("brave")) &&
			!player.isPlaying &&
			(player.position ?? 0) === 0 &&
			(player.length ?? 0) === 0 &&
			!player.trackTitle &&
		...
		if (name === "org.mpris.MediaPlayer2.plasma-browser-integration") {
			if (!player.isPlaying && (player.position ?? 0) === 0 && (player.length ?? 0) === 0) return false;
		}
	...
	function _filterYtMusicDuplicates(playerList) {
		...
		for (let i = 0; i < allPlayers.length; i++) {
			...
			for (let j = i + 1; j < allPlayers.length; j++) {
				...
				// Position/length similarity
				const posMatch =
					(p1.length ?? 0) > 0 &&
					(p2.length ?? 0) > 0 &&
					Math.abs((p1.position ?? 0) - (p2.position ?? 0)) <= 3 &&
					Math.abs((p1.length ?? 0) - (p2.length ?? 0)) <= 3;
	...
	// }
		...
		Connections {
			...
			Component.onDestruction: {
				...
				// Rebuild player list when player is removed
				Qt.callLater(() => {
					root._updateMpvCache();
					root._rebuildPlayerList();
				});
			...
			if (modelData.isPlaying && root.trackedPlayer !== modelData && isRealPlayer(modelData)) {
				root.trackedPlayer = modelData
				root.updateTrack() // ONLY refresh active player
			}
				// DO NOT rebuild the whole player list
			}

			function onTrackArtUrlChanged() {
				if (modelData === root.trackedPlayer)
					root.updateTrack() // Refresh artwork only
			}

			function onMetadataChanged() {
				if (modelData === root.trackedPlayer)
					root.updateTrack() // Refresh title/artist/album only
			}

			function onTrackTitleChanged() {
				if (modelData === root.trackedPlayer)
					root.updateTrack() // Refresh title only
			}
		}
	}
	...
	// 		root.trackedPlayer = YtMusic.mpvPlayer; // update active player
	// 	} else if (this.activePlayer?.desktopEntry?.includes("chromium") ||
	// 			this.activePlayer?.desktopEntry?.includes("firefox")) {
	// 		// Force next/previous for browsers
	// 		this.__reverse = true;
	// 		this.activePlayer.previous();
	// 		root.trackedPlayer = this.activePlayer;
	// 	} else if (this.canGoPrevious) {
	// 		this.__reverse = true;
	// 		this.activePlayer.previous();
	// 		root.trackedPlayer = this.activePlayer;
	// 	} else {
	// 		console.warn("Cannot call previous() on", this.activePlayer, "because canGoPrevious is false.");
	// 	}
	// }
 	...
 	function previous(): void {
        ...
        if (root.isYtMusicActive && YtMusic.currentVideoId) {
            this.__reverse = true
            YtMusic.playPrevious()
            trackedPlayer = YtMusic.mpvPlayer
        } else if (trackedPlayer.desktopEntry?.includes("chromium") ||
                   trackedPlayer.desktopEntry?.includes("firefox")) {
            this.__reverse = true
            trackedPlayer.previous()
            // Do NOT force trackedPlayer update — let onPlaybackStateChanged handle it
        } else if (trackedPlayer?.canGoPrevious) {
            this.__reverse = true
            trackedPlayer.previous()
        } else {
            console.warn("Cannot call previous() on", trackedPlayer, "because canGoPrevious is false.")
        }
    }
	...
	property bool canGoNext: this.activePlayer?.canGoNext ?? false;
		function next(): void {
			if (root.isYtMusicActive && YtMusic.currentVideoId) {
				...
				console.warn("Cannot call next() on", this.activePlayer, "because canGoNext is false.");
			}
		}
