		...
		// Focus
			...
			function updateHighlightAtIndex(index) {
				...
				Keys.onPressed: function(event) {
					...
					if (event.key === Qt.Key_Right && nextIndex < maxIndex) nextIndex++
					else if (event.key === Qt.Key_Left && nextIndex > 0) nextIndex--
					else if (event.key === Qt.Key_Down && nextIndex + cols <= maxIndex) nextIndex += cols
					else if (event.key === Qt.Key_Up && nextIndex - cols >= 0) nextIndex -= cols
					else if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
						// ✅ Use the model data directly
						if (filteredWallpapers.length > 0) {
							applyWallpaper(filteredWallpapers[wallpaperGridView.currentIndex])
						}
						event.accepted = true
						return
					} else return

					updateHighlightAtIndex(nextIndex)
